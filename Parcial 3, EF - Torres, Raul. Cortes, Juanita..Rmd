...
ggplot(data = jp, aes(y = jp, x = index(jp))) + geom_line()+
  ...
  theme_minimal()
```
La serie de la tasa de cambio no presenta una tendencia clara, sin embargo, no es estacionaria
...
ggPacf(jp,52)+ggtitle("")+theme_minimal()
)
```
...
```
En cuanto a la volatilidad de los retornos se observan picos tras la crisis financiera del 2008 
...
```
Finalmente, el valor de la desviación estándar es más alto al de la media, por lo que la volatidad domina el comportamiento de los retornos 
...
resid<-residuals(garch11.fit, standardize=T)
normaldens<-data.frame(predicted = resid, density = dnorm(resid, mean(resid), sd(resid)))

grid.arrange(
ggplot(data= as.data.frame(resid), aes(y =as.data.frame(resid)[,1], x = index(jp_r))) + geom_line()+ggtitle("Residuos del modelo GARCH(1,1)")+ labs(x = "Fecha",y = "") + theme_minimal(),
ggAcf(resid,52)+ggtitle("")+labs(x="")+theme_minimal(),
ggplot(data = as.data.frame(resid), aes(y=..density.., x = jp_r[,1])) +
  geom_histogram(bins = 200) +
  ggtitle("") +
  geom_vline(aes(xintercept=mean(resid)), linetype="dashed") +
  geom_density(alpha=.2, fill="steelblue") + labs(x='',y='') +
  geom_line(aes(y = density), data = normaldens, colour = "firebrick") + theme_minimal(),
ggPacf(resid,52)+ggtitle("")+theme_minimal()
)
```
...
resid<-residuals(garch11sstd.fit, standardize=T)
sstddens<-data.frame(predicted = resid, density = dsstd(resid, mean(resid), sd(resid), xi = garch11sstd.fit@fit[["coef"]][["skew"]]))

grid.arrange(
ggplot(data= as.data.frame(resid), aes(y =as.data.frame(resid)[,1], x = index(jp_r))) + geom_line()+ggtitle("Residuos del modelo GARCH(1,1)~sstd")+ labs(x = "Fecha",y = "") + theme_minimal(),
ggAcf(resid,52)+ggtitle("")+labs(x="")+theme_minimal(),
ggplot(data = as.data.frame(resid), aes(y=..density.., x = jp_r[,1])) +
  geom_histogram(bins = 200) +
  ggtitle("") +
  geom_vline(aes(xintercept=mean(resid)), linetype="dashed") +
  geom_density(alpha=.2, fill="steelblue") + labs(x='',y='') +
  geom_line(aes(y = density), data = sstddens, colour = "firebrick") + theme_minimal(),
ggPacf(resid,52)+ggtitle("")+theme_minimal()
)
```
...
```{r}
resid<-residuals(gjrgarch11.fit, standardize=T)
sstddens<-data.frame(predicted = resid, density = dsstd(resid, mean(resid), sd(resid), xi = gjrgarch11.fit@fit[["coef"]][["skew"]]))

params<-as.list(c(mean(resid), sd(resid), xi = gjrgarch11.fit@fit[["coef"]][["skew"]]))

grid.arrange(
ggplot(data= as.data.frame(resid), aes(y =as.data.frame(resid)[,1], x = index(jp_r))) + geom_line()+ggtitle("Residuos del modelo GJR-GARCH(1,1)~sstd")+ labs(x = "Fecha",y = "") + theme_minimal(),
ggAcf(resid,52)+ggtitle("")+labs(x="")+theme_minimal(),
ggplot(data = as.data.frame(resid), aes(y=..density.., x = jp_r[,1])) +
  geom_histogram(bins = 200) +
  ggtitle("") +
  ...
  geom_density(alpha=.2, fill="steelblue") + labs(x='',y='') +
  geom_line(aes(y = density), data = sstddens, colour = "firebrick") + theme_minimal(),
ggplot(data= as.data.frame(residuals(gjrgarch11.fit, standardize=T)), aes(sample = as.data.frame(resid)[,1])) +
...
ni_r = newsimpact(z = NULL, gjrgarch11.fit)
		...
		xf = ni_r$xexpr
		yf  = ni_r$yexpr
		ggplot(data = data.frame(ni.y_r,ni.x_r), aes(x=ni.x_r,y=ni.y_r))+
		  geom_line(aes(x=ni.x,y=ni.y), colour="steelblue") +
		  geom_line(colour="firebrick") + xlab(xf) + ylab(yf) + theme_minimal()
#		plot( ni.x, ni.y, ylab = yf, xlab = xf, type = "l", lwd = 2, col = "steelblue", main = "News Impact Curve", cex.main = 0.8)
```
