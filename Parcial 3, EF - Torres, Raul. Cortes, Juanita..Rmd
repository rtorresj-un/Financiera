...
```{r, include=FALSE}
library(quantmod); library(PerformanceAnalytics); library(rugarch)
...
library(xts);library(zoo); library(TTR);library(tidyverse);library(dplyr) 
...
ggplot(data = jp, aes(y = `USDJPY=X.Close`, x = index(jp))) + geom_line()

#plot(jp_r, type="l", col="blue", lwd=2, ylab="Adjusted close", main="Retornos diarios")
```

```{r echo=FALSE}
# plot retornos diarios 
ggplot(data = jp_r, aes(y = USDJPY.X.Close, x = fecha)) + geom_line()
```


```{r echo=FALSE, caption='Estadística descriptiva.'}
jp_r<-jp_r[,1]
stats_d<-basicStats(jp_r)[c("Mean", "Stdev", "Minimum", "Maximum", "Variance","Skewness","Kurtosis"),]
