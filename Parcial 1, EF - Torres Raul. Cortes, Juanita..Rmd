...
$$r_t =\mu_t+ \epsilon_t$$
Donde $\mu_t$ representa la media y $\epsilon_t$ el componente aleatorio asociado a la llegada de nueva información al mercado, y como se menciono anteriormente, dado que las expectativas sobre los retornos son racionales, se esperaría que no exista un cambio en los precios.
...
$$\epsilon_t \sim WN \text{    es decir,   } cor(\epsilon_t,\epsilon_s)=0 \text{ con }t \neq s$$   
...
Bajo esta hipótesis,
$$\epsilon_t \sim MD \text{ ,  } \epsilon_t= \sigma_t \eta_t$$
Donde $\sigma_t$ es la volatilidad de los retornos y $\eta_t$ es un proceso aleatorio, lo que implica que $E[\epsilon_t|\epsilon_{t-1},...,\epsilon_{t-n}]=0$ es decir, que el mejor predictor del precio en el siguiente periodo, es el mismo del periodo anterior y la volatilidad de los retornos $(r_t^2)$ es predecible y variable en el tiempo, por lo que se acepta la no linealidad de los retornos.
...
$$\epsilon_t \sim IID(0,\sigma^2)|F_t$$
Lo que signifia que $\epsilon_t$ proviene de una misma distribución y que cada observación en el tiempo es independiente tanto del pasado como de futuro   
